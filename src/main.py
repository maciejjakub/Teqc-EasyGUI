...
class Application(Tk):
    ...
    def check_platform(self):
    	if platform.system() == 'Windows':
    		self.teqc_executable = 'teqc '
    	elif platform.system() == 'Linux':
    		self.teqc_executable = './teqc '

    def footer_text_display(self):
        if platform.system() == 'Windows':
            self.statusbar["text"] = 'File: ' + self.filename.split('\\')[-1] + \
                                            ' successfully opened.'
        elif platform.system() == 'Linux':
            self.statusbar["text"] = 'File: ' + self.filename.split('/')[-1] + \
                                            ' successfully opened.'
    ...
    def show_metadata(self):
        try:
            a = subprocess.run(self.teqc_executable + '+meta ' + self.filename, 
    ...
    def qC(self):
        try:
            ...
            for i in var_list:
                if not i[1]:
                    com_list.append(i[0])
            qc = subprocess.run(self.teqc_executable + '+qc +plot ' + ' '.join(com_list)
                                + self.filename, capture_output=True, encoding='utf-8',
                                shell=True)
    ...
    def qc_full(self):
        try:
            ...
            qc_f = subprocess.run(self.teqc_executable + '+qc +plot -nav ' + ' '.join(com_list) 
                                    + nav_file + ' ' + obs_file, capture_output=True, 
    ...
    def isRinex(self):
        try:
            b = subprocess.run(self.teqc_executable + '+v ' + self.filename, 
                                capture_output=True, encoding='utf-8', shell=True)
    ...
    def chooseFile(self):
        ...
        try:
            check = subprocess.run(self.teqc_executable + self.filename, capture_output=True, 
                                    encoding='utf-8', shell=True)
            while check.stdout == '':
                if self.filename == '':
                    del self.filename
                    ...
                    tkinter.messagebox.showerror('Warning', 'File not selected or selected in \
                                                    incorrect format')
                    self.filename = askopenfilename()
                    check = subprocess.run(self.teqc_executable + self.filename,
                                             capture_output=True, encoding='utf-8', shell=True)
            if self.filename:
                self.footer_text_display()
    ...
    def convert(self):
        try:
            ...
            conv = subprocess.run(self.teqc_executable + input_file + ' > ' + 
                                    self.saved_filename, capture_output=True, 
                                    encoding='utf-8', shell=True)
            print(conv.stderr)
            self.scrltxt.insert(INSERT, conv.stdout)
            self.scrltxt.insert(INSERT, 'Conversion finished successfully.\n')
            self.scrltxt.insert(INSERT, 'Newly created RINEX file ' 
                                + self.saved_filename.split('/')[-1] + ' opened.\n')
            if platform.system() == 'Windows':
    ...
    def convert_with_nav(self):
        try:
            ...
            subprocess.run(self.teqc_executable + '+nav ' + nav_file + ' ' + input_file 
                            + ' > ' + obs_file, capture_output=True, encoding='utf-8', 
                            shell = True)
            self.scrltxt.insert(INSERT, 'Conversion finished successfully.\n')
            self.scrltxt.insert(INSERT, 'Newly created RINEX file ' 
                                + obs_file.split('/')[-1] + ' opened.\n')
            if platform.system() == 'Windows':
    ...
    def format_check(self):
        try:
            format_check = subprocess.run(self.teqc_executable + '+mdf ' + self.filename,
                                         capture_output=True, encoding='utf-8', shell=True)
    ...
    def about_program(self):
        tkinter.messagebox.showinfo('About', 'An application for satellite data editing \
                                    and analyzing based on existing open-source application \
                                    teqc and plotting script teqcplot.py.\nDeveloped by \
                                    Maciej Miliszewski\nWarsaw University of Technology\n\
                                    Poland')
    
    def help(self):
        tkinter.messagebox.showinfo('Help', 'For help with usage of application please \
                                    check out file README.md in source folder')
    ...
    def rinex_header_modify(self):
        try:
            ...
            ('-O.px[WGS84xyz,m] ', self.appposx_entry.get(), self.appposy_entry.get(), 
                                    self.appposz_entry.get()),
            ('-O.pe[hEN,m] ', self.antennadh_entry.get(), self.antennade_entry.get(), 
                                    self.antennadn_entry.get()),
            ...
            subprocess.run(self.teqc_executable + ' '.join(com_list) + ' ' 
                            + self.filename + ' > ' + self.saved_filename, 
                            capture_output=True, encoding='utf-8', shell = True)
            print('teqc ' + ' '.join(com_list) + ' ' + self.filename + ' > '
                     + self.saved_filename)
            self.scrltxt.insert(INSERT, 'Header modification finished successfully.\n')
        except AttributeError:
            self.attribute_error()
    ...
    def compare(self):
        try:
            ...
            compare_variable = subprocess.run('diff ' + file1 + ' ' + file2, 
    ...
    def decimate_data(self):
        try:
            if len(self.interval_entry.get()) > 0 and type(int(self.interval_entry.get())): 
                self.saved_filename = asksaveasfilename()
                subprocess.run(self.teqc_executable + '-O.dec ' + 
                                str(int(self.interval_entry.get())) + ' ' 
                                + self.filename + ' > ' + self.saved_filename, 
                                capture_output=True, encoding='utf-8', shell = True)
                self.scrltxt.insert(INSERT, 'Decimation finished successfully\n')
            else:
                self.attribute_error()
        except AttributeError:
            self.attribute_error()
        except ValueError:
            self.scrltxt.insert(INSERT, 'Cannot decimate data. Inserted value has to \
    ...
    def splice_data(self):
        try:
            ...
            subprocess.run(self.teqc_executable + ' '.join(filename_list) + ' > ' + saved, 
                            encoding='utf-8', shell=True)
            self.scrltxt.insert(INSERT, 'Splicing finished successfully.\n')
        except AttributeError:
            self.attribute_error()
    ...
    def ile(self):
        try:
            ...
            ile = subprocess.run(self.teqc_executable + '-O.sum . ' + ' '.join(com_list)
                                + self.filename, encoding='utf-8', capture_output = True,
                                 shell=True)
            ...
            for i in fileinput.input(files = file):
                if 'antenna WGS 84 (geo)  :  ' in i and float(i.split()[7]) < 180:
                    webbrowser.open('http://www.google.com/maps/place/{},{}'
                                    .format(i.split()[5], i.split()[7]))
                    map_var = 1
                elif 'antenna WGS 84 (geo)  :  ' in i and float(i.split()[7]) > 180:
                    webbrowser.open('http://www.google.com/maps/place/{},{}'
                                    .format(i.split()[5], i.split()[10]))
                    map_var = 1
            if not map_var:
                self.scrltxt3.insert(INSERT, 'Cannot display map, unable to find coordinates \
                                                in quality check report file.\n')
        except FileNotFoundError:
            self.scrltxt3.insert(INSERT, 'Cannot display map, unable to find quality check \
                                            report file.\n')
        except AttributeError:
            self.attribute_error()
    ...
    def draw_skyplot(self):
        try:
            ...
            if int(self.svn_entry.get()):
                draw = subprocess.run('teqcplot.py +skyplot +tcl=' + self.svn_entry.get() 
                                        + ' ' + azimuth_file + ' ' + elevation_file 
                                        + ' ' + self.drawfile, capture_output=True, 
                                        encoding='utf-8', shell = True)
    ...
    def draw_bandplot(self):
        try:
            ...
            print(self.svn_entry.get())
            if int(self.svn_entry.get()):
                draw = subprocess.run('teqcplot.py +bandplot +tcl=' + self.svn_entry.get() 
                                        + ' ' + azimuth_file + ' ' + elevation_file 
                                        + ' ' + self.drawfile, capture_output=True, 
                                        encoding='utf-8', shell = True)
                self.scrltxt3.insert(INSERT, draw.stdout)
    ...
    def draw_azelplot(self):
        try:
            ...
            print(self.svn_entry.get())
            if int(self.svn_entry.get()):
                draw = subprocess.run('teqcplot.py +azelplot +tcl=' + self.svn_entry.get() 
                                        + ' ' + azimuth_file + ' ' + elevation_file 
                                        + ' ' + self.drawfile, capture_output=True, 
                                        encoding='utf-8', shell = True)
                self.scrltxt3.insert(INSERT, draw.stdout)
    ...
    def draw_timeelplot(self):
        try:
            ...
            print(self.svn_entry.get())
            if int(self.svn_entry.get()):
                draw = subprocess.run('teqcplot.py +timeelplot +tcl=' + self.svn_entry.get() 
                                        + ' ' + azimuth_file + ' ' + elevation_file 
                                        + ' ' + self.drawfile, capture_output=True, 
                                        encoding='utf-8', shell = True)
                self.scrltxt3.insert(INSERT, draw.stdout)
